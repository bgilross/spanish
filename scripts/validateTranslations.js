...
function buildVocabSet() {
	...
	function collect(obj) {
		...
		if (obj.word && typeof obj.word === "string")
			set.add(stripAccents(obj.word.toLowerCase()))
...
function resolveReferenceKey(refKey) {
	...
	if (typeof cur === "object" && cur.word)
		return stripAccents(String(cur.word).toLowerCase())
	return null
}
...
function runValidation() {
	...
	for (const lesson of spanishData.lessons || []) {
		for (const s of lesson.sentences || []) {
			...
			if (missing.length)
				issues.push(
					`Tokens present in sentence translation but NOT found in any section translations: ${missing.join(
						", "
					)}`
				)
			...
			if (extra.length)
				issues.push(
					`Tokens present in section translations but NOT found in sentence translation: ${extra.join(
						", "
					)}`
				)
