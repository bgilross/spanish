...
function stripAccents(s) { if (!s) return s; return s.normalize('NFD').replace(/\p{Diacritic}/gu, '').normalize() }

function buildWordMapFromWordsText(text) {
  const map = new Map()
  // Look for objects that contain id: "..." and word: "..." within the same object
  const re = /id\s*:\s*["']([a-zA-Z0-9_.]+)["'][\s\S]{0,200}?word\s*:\s*["']([^"']+)["']/g
  let m
  ...
  const re2 = /([a-zA-Z0-9_.]+)\s*:\s*\{[\s\S]{0,200}?word\s*:\s*["']([^"']+)["']/g
...
function extractEntriesForSentences(text) {
  ...
  for (let i=0;i<lines.length;i++){
    const line = lines[i]
    const trMatch = line.match(/translation:\s*(["'])([^"']+)\1/) || line.match(/translation:\s*([A-Z0-9_\.]+)/)
    if (trMatch) {
      const translation = trMatch[2] || trMatch[1]
      let id = null; let sentence = ''
      for (let j=i-8;j<=i;j++){
        if (j<0) continue
        const l = lines[j]
      ...
      while (k < lines.length) {
        ...
        if (p || refKeys.length || tkeys.length) dataEntries.push({ phrase: p ? p[2] : null, refKeys, transKeys: tkeys })
        k++
      }
...
const COMMON_TOKENS = new Set(['que','eso','de','a','no','el','la','lo','los','las','por','para','con','en','ser','soy','eres','somos','son','me','te','se','un','una','y','o'])
...
for (const r of results) {
  ...
  if (missing.length || extra.length) report.push({ lesson: null, id: r.id, sentence: r.sentence, missing, extra })
}
...
for (const r of report) {
  ...
  if (r.missing.length) console.log('  Missing tokens in sections:', r.missing.join(', '))
  if (r.extra.length) console.log('  Extra tokens in sections:', r.extra.join(', '))
}
